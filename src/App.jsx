...
const Activity = ({ user, onDisc, setPM }) => {
  ...
  const waData = {
    ...
    groups: [{ from: "Family Group", prev: "Dad: See everyone Sunday 🙌", time: "Yesterday", type: "groups" }],
  };
        ...
        {!act && cur?.lk && (
          <div style={{ padding: "28px 20px" }}>
            <div style={{ textAlign: "center", marginBottom: 26 }}>
              <h2 className="raj" style={{ fontSize: 22, fontWeight: 700, marginBottom: 6 }}>Your Data Streams</h2>
              <p style={{ color: MUTED, fontSize: 13, maxWidth: 380, margin: "0 auto" }}>All 9+ streams are ready to sync — activate a plan to unlock them.</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 18, maxWidth: 900, margin: "0 auto 26px" }}>
              {streams.map(s => {
                ...
                const cl = colors[s.id] || GOLD;
                return (
                  <div key={s.id} className="glass" style={{ padding: 22, borderRadius: 18, borderColor: cl + "30", background: `linear-gradient(135deg,${cl}10,${cl}04)` }}>
            ...
            <div style={{ textAlign: "center" }}>
              {user?.profile?.payment_status === "pending" ? (<div className="tag tg" style={{ justifyContent: "center", padding: "10px 22px", display: "inline-flex" }}>⏳ Pending approval</div>) : (<button className="btn-gold" onClick={() => setPM(true)}><Ic n="zap" sz={15} />Activate — from $79</button>)}
            </div>
          </div>
        )}
