...
// Convert an uploaded icon/logo image to a base64 data URL (no Storage bucket required).
// Used for custom payment method icons in Admin → Site Settings → Payment Methods.
const readIconAsDataURL = (file) => {
  if (!file.type.startsWith("image/")) throw new Error("Please upload an image file.");
  if (file.size > 1 * 1024 * 1024) throw new Error("Icon too large. Please upload an image under 1MB.");
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result);
    reader.onerror = () => reject(new Error("Could not read file. Please try again."));
...
const PayModal = ({ onClose, onOk, user, token }) => {
  ...
  const [sitePlans, setSitePlans] = useState(null); // admin-configured plans, null until loaded

  // Pull admin-configured WhatsApp/Facebook support links, Payment Methods
  // settings, and the admin's actual Plans (pricing/features) so this modal
  // always matches Admin → Settings instead of showing stale hardcoded values.
  useEffect(() => {
    const loadSupportLinks = async () => {
      ...
      try {
        const rows = await supa("site_settings?key=eq.plans&select=value", "GET", null, token);
        if (rows?.[0]?.value) { const p = JSON.parse(rows[0].value); if (Array.isArray(p) && p.length) setSitePlans(p); }
      } catch(e) {}
    };
    loadSupportLinks();
  }, []);
  ...
  const customMethods = (payCfg?.custom || []).filter(m => m.enabled && m.label).map(m => ({ id: m.id, lb: m.label, icon: m.icon, custom: true }));
  const methods = [...(payCfg ? ALL_METHODS.filter(m => payCfg[m.id]?.enabled !== false) : ALL_METHODS), ...customMethods];
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (user?.id) {
        const payAmt = plans.find(p => p.name === plan)?.price ?? 0;
  ...
  return (
    <div className="mo" onClick={onClose}>
      <div className="md" onClick={e => e.stopPropagation()} style={{ maxWidth: 580 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, marginBottom: 18 }}>
          {methods.map(m => (
            <div key={m.id} onClick={() => { setMeth(m.id); setGc(null); }} style={{ padding: "13px 8px", borderRadius: 11, border: `2px solid ${meth === m.id ? GOLD : "rgba(201,168,76,0.15)"}`, background: meth === m.id ? "rgba(201,168,76,0.08)" : "rgba(255,255,255,0.02)", cursor: "pointer", transition: "all .2s", textAlign: "center" }}>
              {m.icon ? (
                <img src={m.icon} alt="" style={{ width: 28, height: 28, borderRadius: 7, objectFit: "cover", margin: "0 auto 6px", display: "block" }} />
              ) : m.badge ? (
        ...
        {(() => {
          const cm = methods.find(m => m.id === meth)?.custom ? payCfg?.custom?.find(c => c.id === meth) : null;
          if (!cm) return null;
          return (
            <div className="ad" style={{ marginBottom: 18 }}>
              <p style={{ fontSize: 12, color: MUTED, marginBottom: 12 }}>Send exact amount, then email your proof to <span style={{ color: GOLD }}>trustcotech0@gmail.com</span></p>
              {cm.handle ? (
                <div className="wb" onClick={() => copy(cm.handle, "custom_" + meth)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 9 }}>
                    {cm.icon ? <img src={cm.icon} alt="" style={{ width: 34, height: 34, borderRadius: 9, objectFit: "cover" }} /> : <div style={{ width: 34, height: 34, borderRadius: 9, background: "rgba(201,168,76,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}><Ic n="credit" sz={16} c={GOLD} /></div>}
                    <div><p className="raj" style={{ fontWeight: 700, fontSize: 15 }}>{cm.label}</p><p style={{ fontSize: 10, color: MUTED }}>Click to copy</p></div>
                    <div style={{ marginLeft: "auto" }}><span className="tag tg" style={{ fontSize: 9 }}>{cp === "custom_" + meth ? "✓ Copied!" : "Copy"}</span></div>
                  </div>
                  <p className="mono" style={{ fontSize: 11, color: GOLD, wordBreak: "break-all", lineHeight: 1.6 }}>{cm.handle}</p>
                </div>
              ) : (
                <>
                  <p style={{ fontSize: 12, color: MUTED, marginBottom: 11 }}>Message us your name and chosen plan and we'll send you the {cm.label} details to complete payment.</p>
                  <a href={`${waUrl}${waUrl.includes("?") ? "&" : "?"}text=${encodeURIComponent(`Hi, I'd like to pay for the ${plan || ""} plan via ${cm.label}. My email is ${user?.email || ""}`)}`} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "block" }}>
        ...
        {((meth === "giftcard" && gc) || meth === "crypto" || CONTACT_METHODS.includes(meth) || methods.find(m => m.id === meth)?.custom) && plan && (
          <div>
        ...
        {((meth === "giftcard" && gc) || meth === "crypto" || CONTACT_METHODS.includes(meth) || methods.find(m => m.id === meth)?.custom) && plan ? (
          <button className="btn-gold" style={{ width: "100%", justifyContent: "center", fontSize: 15, marginTop: 8 }} onClick={handleConfirm} disabled={saving || proofUploading}>
            <Ic n="check" sz={16} />{saving ? "Saving..." : meth === "giftcard" ? `I've sent the ${gc} code →` : meth === "crypto" ? "I've sent the crypto payment →" : `I've sent the payment via ${methLabel(meth)} →`}
          </button>
        ) : (
...
const AdminDash = ({ user, setUser, sp }) => {
  ...
  const DEFAULT_PAY_METHODS = {
    ...
    custom: [], // admin-added methods: [{ id, label, icon (base64), handle, enabled }]
  };
  const DEFAULT_SITE_CONTENT = {
    ...
    steps_title: "Connect in 3 simple steps",
    step1_title: "Receive Partner ID",
    step1_desc: "Our admin issues a unique Partner ID to your account within 10 mins.",
    step2_title: "Connect",
    step2_desc: "Enter the Partner ID to connect to your partner device.",
    step3_title: "Choose Your Plan",
    step3_desc: "Select a subscription that fits your needs and complete payment via your preferred method, to instantly access calls, messages, location and more.",
  ...
  const load = async () => {
    ...
    try {
      ...
      if (s && s.length > 0) {
        ...
        if (settings.payment_methods) { try { setSitePayMethods(p => ({ ...p, ...JSON.parse(settings.payment_methods) })); } catch(e) {} }
            ...
            {settingsTab === "payment" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div className="glass" style={{ padding: 24 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {Object.entries(sitePayMethods).filter(([key]) => key !== "custom").map(([key, m]) => (
                  ...
                  {/* CUSTOM PAYMENT METHODS — for anything not in the fixed list above,
                      with its own uploaded icon/logo. */}
                  <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <h4 className="raj" style={{ fontSize: 14, fontWeight: 700, color: GOLD, marginBottom: 4 }}>Custom Payment Methods</h4>
                    <p style={{ fontSize: 11, color: MUTED, marginBottom: 14 }}>Add any other payment method not listed above — upload its logo, set a label and your handle/instructions.</p>
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      {(sitePayMethods.custom || []).map((m, i) => (
                        <div key={m.id} style={{ padding: "14px 16px", background: m.enabled ? "rgba(201,168,76,0.04)" : "rgba(255,255,255,0.02)", border: `1px solid ${m.enabled ? "rgba(201,168,76,0.2)" : "rgba(255,255,255,0.06)"}`, borderRadius: 10 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                            <label style={{ cursor: "pointer", flexShrink: 0 }}>
                              <input type="file" accept="image/*" style={{ display: "none" }} onChange={async e => {
                                const file = e.target.files?.[0]; if (!file) return;
                                try {
                                  const dataUrl = await readIconAsDataURL(file);
                                  const list = [...(sitePayMethods.custom || [])]; list[i] = { ...list[i], icon: dataUrl };
                                  setSitePayMethods(p => ({ ...p, custom: list }));
                                } catch (err) { showToast(err.message || "Could not upload icon"); }
                              }} />
                              <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px dashed rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                                {m.icon ? <img src={m.icon} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Ic n="img" sz={16} c={MUTED} />}
                              </div>
                            </label>
                            <input className="inp" style={{ fontSize: 12, flex: 1 }} placeholder="Method name e.g. Wise, Skrill" value={m.label} onChange={e => { const list = [...sitePayMethods.custom]; list[i] = { ...list[i], label: e.target.value }; setSitePayMethods(p => ({ ...p, custom: list })); }} />
                            <div onClick={() => { const list = [...sitePayMethods.custom]; list[i] = { ...list[i], enabled: !list[i].enabled }; setSitePayMethods(p => ({ ...p, custom: list })); }} style={{ width: 40, height: 22, borderRadius: 11, background: m.enabled ? GOLD : "rgba(255,255,255,0.1)", cursor: "pointer", position: "relative", transition: "background .2s", flexShrink: 0 }}>
                              <div style={{ position: "absolute", top: 2, left: m.enabled ? 20 : 2, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left .2s" }} />
                            </div>
                            <button onClick={() => { const list = sitePayMethods.custom.filter((_, idx) => idx !== i); setSitePayMethods(p => ({ ...p, custom: list })); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#FF4D6A", padding: 4, flexShrink: 0 }}><Ic n="trash" sz={15} /></button>
                          </div>
                          <input className="inp" style={{ fontSize: 12 }} placeholder="Your handle / username / instructions for this method" value={m.handle || ""} onChange={e => { const list = [...sitePayMethods.custom]; list[i] = { ...list[i], handle: e.target.value }; setSitePayMethods(p => ({ ...p, custom: list })); }} />
                        </div>
                      ))}
                    </div>
                    <button className="btn-out" style={{ marginTop: 12, fontSize: 12, padding: "9px 16px" }} onClick={() => { const id = `custom_${Date.now()}`; setSitePayMethods(p => ({ ...p, custom: [...(p.custom || []), { id, label: "", icon: "", handle: "", enabled: true }] })); }}><Ic n="plus" sz={13} />Add Payment Method</button>
                  </div>
            ...
            {settingsTab === "content" && (
              <div className="glass" style={{ padding: 28 }}>
                ...
                <h4 className="raj" style={{ fontSize: 13, fontWeight: 700, color: GOLD, marginBottom: 4, marginTop: 4 }}>Landing Page — Top Banner</h4>
                  ...
                  <h4 className="raj" style={{ fontSize: 13, fontWeight: 700, color: GOLD, marginBottom: 4, marginTop: 10 }}>Landing Page — "How It Works" (3 Steps)</h4>
                  <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Section Eyebrow</label><input className="inp" value={siteContent.steps_eyebrow || ""} onChange={e => setSiteContent(p => ({ ...p, steps_eyebrow: e.target.value }))} /></div>
                  <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Section Title</label><input className="inp" value={siteContent.steps_title || ""} onChange={e => setSiteContent(p => ({ ...p, steps_title: e.target.value }))} /></div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 10 }}>
                    <p style={{ fontSize: 11, color: GOLD, fontWeight: 700 }}>Step 1</p>
                    <input className="inp" style={{ fontSize: 13 }} placeholder="Title" value={siteContent.step1_title || ""} onChange={e => setSiteContent(p => ({ ...p, step1_title: e.target.value }))} />
                    <textarea className="inp" style={{ fontSize: 12, resize: "vertical" }} rows={2} placeholder="Description" value={siteContent.step1_desc || ""} onChange={e => setSiteContent(p => ({ ...p, step1_desc: e.target.value }))} />
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 10 }}>
                    <p style={{ fontSize: 11, color: GOLD, fontWeight: 700 }}>Step 2</p>
                    <input className="inp" style={{ fontSize: 13 }} placeholder="Title" value={siteContent.step2_title || ""} onChange={e => setSiteContent(p => ({ ...p, step2_title: e.target.value }))} />
                    <textarea className="inp" style={{ fontSize: 12, resize: "vertical" }} rows={2} placeholder="Description" value={siteContent.step2_desc || ""} onChange={e => setSiteContent(p => ({ ...p, step2_desc: e.target.value }))} />
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 10 }}>
                    <p style={{ fontSize: 11, color: GOLD, fontWeight: 700 }}>Step 3</p>
                    <input className="inp" style={{ fontSize: 13 }} placeholder="Title" value={siteContent.step3_title || ""} onChange={e => setSiteContent(p => ({ ...p, step3_title: e.target.value }))} />
                    <textarea className="inp" style={{ fontSize: 12, resize: "vertical" }} rows={2} placeholder="Description" value={siteContent.step3_desc || ""} onChange={e => setSiteContent(p => ({ ...p, step3_desc: e.target.value }))} />
                  </div>
...
const Home = ({ sp, reviews, setReviews, user }) => {
  ...
  const [homePlans, setHomePlans] = useState([
    { name: "3 Months", price: 79, ft: ["All 9 data streams", "1 device", "Email support"] },
    { name: "6 Months", price: 100, ft: ["All 9 data streams", "2 devices", "Email support"] },
    { name: "2 Years", price: 300, best: true, ft: ["All 9 data streams", "Up to 5 devices", "Priority support", "Free updates"] },
  ]);
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        const rows = await supa("site_settings?key=eq.plans&select=value", "GET");
        if (rows?.[0]?.value) {
          const p = JSON.parse(rows[0].value);
          if (Array.isArray(p) && p.length) setHomePlans(p.map(pl => ({ ...pl, ft: pl.ft || pl.features || [] })));
        }
      } catch(e) {}
    };
    loadContent();
  ...
  return (
    <div>
      ...
      <section className="sec"><div className="ct">
        <div style={{ textAlign: "center", marginBottom: 60 }}><div className="ew" style={{ justifyContent: "center" }}>{sc.steps_eyebrow}</div><h2 className="raj" style={{ fontSize: "clamp(26px,4vw,46px)", fontWeight: 700 }}>{sc.steps_title}</h2></div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(270px,1fr))", gap: 22 }}>
          {[{ n: "01", ic: "key", cl: GOLD, t: sc.step1_title, d: sc.step1_desc }, { n: "02", ic: "link", cl: SILVER, t: sc.step2_title, d: sc.step2_desc }, { n: "03", ic: "credit", cl: GOLD, t: sc.step3_title, d: sc.step3_desc }].map(s => (
...
const Pricing = ({ sp }) => {
  ...
  const [plans, setPlans] = useState([
    { name: "3 Months", price: 79, period: "3-month licence", ft: ["All 9 data streams", "1 device linked", "Calls, SMS & WhatsApp", "Instagram & Facebook", "Photos, Videos & Contacts", "Live Location", "Email support", "Admin-issued Partner ID"] },
    { name: "6 Months", price: 100, period: "6-month licence", ft: ["All 9 data streams", "2 devices linked", "Calls, SMS & WhatsApp", "Instagram & Facebook", "Photos, Videos & Contacts", "Live Location", "Email support", "Admin-issued Partner ID"] },
    { name: "2 Years", price: 300, period: "2-year licence", best: true, ft: ["All 9 data streams", "Up to 5 devices", "Everything in 6 months", "Priority 24/7 support", "Free app updates", "Early feature access", "Admin-issued Partner ID"] },
  ]);
  // Pull admin-configured plans (Admin → Site Settings → Plans) so pricing
  // shown here always matches what admin actually set.
  useEffect(() => {
