...
const AdminDash = ({ user, setUser, sp }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [u, p, r, pay, t] = await Promise.all([
        supa("profiles?select=*&order=created_at.desc", "GET", null, user.token),
        supa("partner_ids?select=*&order=created_at.desc", "GET", null, user.token),
        supa("reviews?select=*&order=created_at.desc", "GET", null, user.token),
        supa("payments?select=*&order=created_at.desc", "GET", null, user.token),
        supa("tickets?select=*&order=created_at.desc", "GET", null, user.token),
      ]);
      setUsers(u || []); setPids(p || []); setReviews(r || []); setPayments(pay || []); setTickets(t || []);
...
const Login = ({ sp, su }) => {
  ...
  const go = async () => {
    ...
    try {
      // Check admin
      if (f.e === ADMIN_EMAIL && f.p === "Base100@") {
        const data = await supaAuth("token?grant_type=password", { email: f.e, password: f.p });
        su({ id: data.user.id, email: f.e, token: data.access_token, isAdmin: true });
        sp("admin"); return;
      }
      const data = await supaAuth("token?grant_type=password", { email: f.e, password: f.p });
      // Load profile
      const profiles = await supa(`profiles?id=eq.${data.user.id}&select=*`, "GET", null, data.access_token);
      su({ id: data.user.id, email: data.user.email, token: data.access_token, profile: profiles?.[0] || {} });
      sp("dashboard");
...
const Register = ({ sp, su }) => {
  ...
  const go = async () => {
    ...
    try {
      ...
      // Create profile
      try {
        await supa("profiles", "POST", { id: data.user.id, name: f.n, email: f.e, plan: "free", activated: false });
      } catch (e) { }
      su({ id: data.user.id, email: data.user.email, token: data.access_token, profile: { name: f.n, email: f.e, activated: false } });
      sp("dashboard");
...
export default function App() {
  ...
  const sp = page => {
    if (!PUBLIC_PAGES.includes(page) && !user) { setPg("login"); return; }
    setPg(page);
    window.scrollTo(0, 0);
  };
