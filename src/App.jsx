...
// ============================================================
const RESEND_API_KEY = "re_gp8TZ8T3_8qqKJPdnUFaTy3oip1u4ecNa";
const FROM_EMAIL    = "TrustCo Tech <noreply@trustcotech.xyz>";
...
const sanitize = (str) => {
  ...
  return str
    ...
    .replace(/['"`;]/g, "")                        // block SQL-style chars
    .trim()
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (user?.id) {
        ...
        await supa(`profiles?id=eq.${user.id}`, "PATCH", { payment_status: "pending" }, token).catch(() => {});
      ...
      <div className="md" onClick={e => e.stopPropagation()} style={{ maxWidth: 580 }}>
        ...
        {CONTACT_METHODS.includes(meth) && (
          <div className="ad" style={{ marginBottom: 18 }}>
            ...
            <a href={FB_URL} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "block", marginBottom: 11 }}>
    ...
    <div style={{ display: "flex", height: "calc(100vh - 70px)", position: "relative" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", background: DARK, position: "relative" }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          {tab === "whatsapp_biz" && !cur?.lk && (<div className="ad"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 className="raj" style={{ fontSize: 20, fontWeight: 700 }}>WhatsApp Business</h2><span className="tag tgg"><span className="dot on" style={{ width: 6, height: 6 }} /> Synced</span></div>{renderSubTabs()}{[{ from:"Luxe Boutique 💼", prev:"Your order #4821 is ready for pickup!", time:"11:02 AM", av:"L", type:"received"}, { from:"FreshMart Grocery 💼", prev:"Flash sale ends at midnight — 30% off all orders", time:"10:14 AM", av:"F", type:"received"}, { from:"DriveEasy Support 💼", prev:"Your ride receipt: $14.50 — Tap to view", time:"9:48 AM", av:"D", type:"received"}, { from:"ZenFit Studio 💼", prev:"Reminder: Yoga class tomorrow at 7:00 AM", time:"Yesterday", av:"Z", type:"received"}, { from:"You", prev:"Can I reschedule to 9 AM instead?", time:"Yesterday", av:"Y", type:"sent"}].filter(m => sub==="sent"?m.type==="sent":sub==="received"?m.type==="received":true).map((m, i) => (<div key={i} className="glass" style={{ padding: "13px 17px", display: "flex", gap: 12, alignItems: "center", marginBottom: 7, cursor: "pointer" }}><div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#128C7E,#075E54)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#fff", flexShrink: 0 }}>{(m.av||m.from[0])}</div><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>{m.from}</span><div style={{ display: "flex", gap: 6 }}><span className={`tag ${m.type==="sent"?"tgs":"tgg"}`} style={{ fontSize: 9 }}>{m.type}</span><span style={{ fontSize: 11, color: MUTED }}>{m.time}</span></div></div><p style={{ color: MUTED, fontSize: 12, marginTop: 2 }}>{m.prev}</p></div></div>))}<div className="glass" style={{ padding: 18, marginTop: 14 }}><p style={{ fontSize: 11, color: MUTED, marginBottom: 12, textTransform: "uppercase", letterSpacing: ".08em" }}>Latest — Luxe Boutique 💼</p><div style={{ display: "flex", flexDirection: "column", gap: 9 }}><div style={{ display: "flex" }}><div className="bub bi">Hi! Your order #4821 is ready 📦</div></div><div style={{ display: "flex" }}><div className="bub bi">You can collect it any time before 8 PM today 🕗</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">Great, I'll pick it up around 5!</div></div><div style={{ display: "flex" }}><div className="bub bi">Perfect! We'll keep it at the front desk 👍</div></div></div></div></div>)}
          {tab === "snapchat" && !cur?.lk && (<div className="ad"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 className="raj" style={{ fontSize: 20, fontWeight: 700 }}>Snapchat</h2><span className="tag tgg"><span className="dot on" style={{ width: 6, height: 6 }} /> Synced</span></div>{renderSubTabs()}{[{ from:"tyler_snap", prev:"😂 opened a snap", time:"11:15 AM", av:"T", type:"received"}, { from:"mia.snaps", prev:"🔥 sent you a snap", time:"10:48 AM", av:"M", type:"received"}, { from:"you", prev:"📷 you sent a snap", time:"10:50 AM", av:"Y", type:"sent"}, { from:"jake.w", prev:"💬 bro did you see that?? 😭", time:"Yesterday", av:"J", type:"received"}, { from:"ava_moments", prev:"⭕ viewed your story", time:"Yesterday", av:"A", type:"received"}].filter(m => sub==="sent"?m.type==="sent":sub==="received"?m.type==="received":sub==="stories"?m.prev.includes("⭕"):sub==="memories"?false:true).map((m, i) => (<div key={i} className="glass" style={{ padding: "13px 17px", display: "flex", gap: 12, alignItems: "center", marginBottom: 7, cursor: "pointer" }}><div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#FFFC00,#F5D800)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#333", flexShrink: 0 }}>{(m.av||m.from[0])}</div><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>{m.from}</span><div style={{ display: "flex", gap: 6 }}><span className={`tag ${m.type==="sent"?"tgs":"tgg"}`} style={{ fontSize: 9 }}>{m.type}</span><span style={{ fontSize: 11, color: MUTED }}>{m.time}</span></div></div><p style={{ color: MUTED, fontSize: 12, marginTop: 2 }}>{m.prev}</p></div></div>))}<div className="glass" style={{ padding: 18, marginTop: 14 }}><p style={{ fontSize: 11, color: MUTED, marginBottom: 12, textTransform: "uppercase", letterSpacing: ".08em" }}>Latest — jake.w 👻</p><div style={{ display: "flex", flexDirection: "column", gap: 9 }}><div style={{ display: "flex" }}><div className="bub bi">bro did you see that?? 😭</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">lmao no what happened 💀</div></div><div style={{ display: "flex" }}><div className="bub bi">check my story rn it's insane</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">omg 😭😭 no way</div></div></div></div></div>)}
          {tab === "telegram" && !cur?.lk && (<div className="ad"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 className="raj" style={{ fontSize: 20, fontWeight: 700 }}>Telegram</h2><span className="tag tgg"><span className="dot on" style={{ width: 6, height: 6 }} /> Synced</span></div>{renderSubTabs()}{[{ from:"Lena K.", prev:"Did you check the flight prices?", time:"11:30 AM", av:"L", type:"received"}, { from:"Dev Team 🛠️", prev:"Carlos: PR approved, merging now", time:"10:55 AM", av:"D", type:"groups"}, { from:"You", prev:"Yeah, found one for $340 — looks good", time:"11:33 AM", av:"Y", type:"sent"}, { from:"CryptoAlerts 📢", prev:"BTC just crossed $72,000 📈", time:"9:47 AM", av:"C", type:"received"}, { from:"Marcus T.", prev:"Call me when you're free 🤙", time:"Yesterday", av:"M", type:"received"}].filter(m => sub==="sent"?m.type==="sent":sub==="received"?m.type==="received":sub==="groups"?m.type==="groups":sub==="channels"?m.from.includes("📢")||m.from.includes("📰")||m.from.includes("💹"):true).map((m, i) => (<div key={i} className="glass" style={{ padding: "13px 17px", display: "flex", gap: 12, alignItems: "center", marginBottom: 7, cursor: "pointer" }}><div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#2AABEE,#1d8fc4)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#fff", flexShrink: 0 }}>{(m.av||m.from[0])}</div><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>{m.from}</span><div style={{ display: "flex", gap: 6 }}><span className={`tag ${m.type==="sent"?"tgs":m.type==="groups"?"tgb":"tgg"}`} style={{ fontSize: 9 }}>{m.type}</span><span style={{ fontSize: 11, color: MUTED }}>{m.time}</span></div></div><p style={{ color: MUTED, fontSize: 12, marginTop: 2 }}>{m.prev}</p></div></div>))}<div className="glass" style={{ padding: 18, marginTop: 14 }}><p style={{ fontSize: 11, color: MUTED, marginBottom: 12, textTransform: "uppercase", letterSpacing: ".08em" }}>Latest — Lena K. ✈️</p><div style={{ display: "flex", flexDirection: "column", gap: 9 }}><div style={{ display: "flex" }}><div className="bub bi">Did you check the flight prices? ✈️</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">Yeah, found one for $340 — looks good</div></div><div style={{ display: "flex" }}><div className="bub bi">Book it!! I'm so ready for this trip 🌍</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">Done! Confirmation sent to your email 📧</div></div></div></div></div>)}
          {tab === "tiktok" && !cur?.lk && (<div className="ad"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 className="raj" style={{ fontSize: 20, fontWeight: 700 }}>TikTok DMs</h2><span className="tag tgg"><span className="dot on" style={{ width: 6, height: 6 }} /> Synced</span></div>{renderSubTabs()}{[{ from:"@mikayla.dances", prev:"omg your last video was so good!! 🔥", time:"11:44 AM", av:"M", type:"received"}, { from:"@devonte_clips", prev:"collab? lmk 👀", time:"10:20 AM", av:"D", type:"received"}, { from:"@you", prev:"thanks!! means a lot 🙏", time:"11:46 AM", av:"Y", type:"sent"}, { from:"@zara.aesthetic", prev:"which filter did you use for that?", time:"Yesterday", av:"Z", type:"received"}, { from:"@noahvlogs", prev:"just followed you! check out my page 🤍", time:"Yesterday", av:"N", type:"received"}].filter(m => sub==="sent"?m.type==="sent":sub==="received"?m.type==="received":sub==="videos"?false:true).map((m, i) => (<div key={i} className="glass" style={{ padding: "13px 17px", display: "flex", gap: 12, alignItems: "center", marginBottom: 7, cursor: "pointer" }}><div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#FF0050,#c4003c)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#fff", flexShrink: 0 }}>{(m.av||m.from[0])}</div><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>{m.from}</span><div style={{ display: "flex", gap: 6 }}><span className={`tag ${m.type==="sent"?"tgs":"tgg"}`} style={{ fontSize: 9 }}>{m.type}</span><span style={{ fontSize: 11, color: MUTED }}>{m.time}</span></div></div><p style={{ color: MUTED, fontSize: 12, marginTop: 2 }}>{m.prev}</p></div></div>))}{sub==="videos"&&(<div style={{ display:"flex", flexDirection:"column", gap:9, marginTop:8 }}>{[{from:"@chef.rina",prev:"pasta recipe for beginners 🍝",time:"2h ago"},{from:"@traveljosh",prev:"48hrs in Tokyo 🗾",time:"3h ago"},{from:"@gymguru.official",prev:"morning routine 💪",time:"Yesterday"},{from:"@noahvlogs",prev:"day in my life nyc 🗽",time:"Yesterday"}].map((v,i)=>(<div key={i} className="glass" style={{ padding:"13px 17px", display:"flex", gap:12, alignItems:"center" }}><div style={{ width:40, height:40, borderRadius:"50%", background:"linear-gradient(135deg,#FF0050,#c4003c)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>❤️</div><div style={{ flex:1 }}><span style={{ fontWeight:600, fontSize:13 }}>{v.from}</span><p style={{ color:MUTED, fontSize:12, marginTop:2 }}>{v.prev}</p></div><span style={{ fontSize:11, color:MUTED }}>{v.time}</span></div>))}</div>)}<div className="glass" style={{ padding: 18, marginTop: 14 }}><p style={{ fontSize: 11, color: MUTED, marginBottom: 12, textTransform: "uppercase", letterSpacing: ".08em" }}>Latest — @mikayla.dances 🎵</p><div style={{ display: "flex", flexDirection: "column", gap: 9 }}><div style={{ display: "flex" }}><div className="bub bi">omg your last video was so good!! 🔥</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">thanks!! means a lot 🙏</div></div><div style={{ display: "flex" }}><div className="bub bi">how long did it take you to edit??</div></div><div style={{ display: "flex", justifyContent: "flex-end" }}><div className="bub bo">like 2 hours lol, capcut is life 😭</div></div></div></div></div>)}
...
const UserDash = ({ user, setUser, sp }) => {
  ...
  const [flowStep, setFlow] = useState(0);
  ...
  const deleteAccount = async () => {
    ...
    setDeleting(true);
    try {
      await supa(`profiles?id=eq.${user.id}`, "DELETE", null, user.token);
    } catch (e) { console.log(e); }
    setUser(null); sp("home");
  };
  ...
  const loadPayments = async () => {
    ...
    setPayLoading(true);
    try {
      const p = await supa(`payments?user_id=eq.${user.id}&select=*&order=created_at.desc`, "GET", null, user.token);
      setPayments(p || []);
    } catch (e) { console.log(e); }
    setPayLoading(false);
  };
  ...
  // Refresh profile from DB on load to get latest activated/plan/partner_id status
  useEffect(() => {
  ...
  const FlowPartnerID = () => {
    ...
    const go = () => {
      ...
      if (!assignedId) { setErr("No Partner ID has been assigned to your account yet. Please contact admin."); setShk(true); setTimeout(() => setShk(false), 500); return; }
    ...
    const flowContent = user?.profile?.flow_partner || {};
    return (
      <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
        <div style={{ width: "100%", maxWidth: 480 }}>
          <div className="glass au" style={{ padding: "36px 28px" }}>
            <div style={{ textAlign: "center", marginBottom: 28 }}>
              <div style={{ width: 64, height: 64, borderRadius: 18, background: "linear-gradient(135deg,#9A7B2F,#F0D080)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28 }}>🔑</div>
              <h2 className="raj" style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Enter Partner ID</h2>
              <p style={{ color: MUTED, fontSize: 13.5, lineHeight: 1.7 }}>Enter the Partner ID assigned to your account to connect your second device.</p>
            </div>
            {/* Note box */}
            <div style={{ padding: "13px 16px", background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 10, marginBottom: 22, display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ fontSize: 16, flexShrink: 0 }}>💬</span>
              <p style={{ fontSize: 12.5, color: "#C9A84C", lineHeight: 1.6 }}>
                Your Partner ID is issued by admin. If you don't have one yet, contact support{" "}
                <a href={FB_URL} target="_blank" rel="noreferrer" style={{ color: GOLD, fontWeight: 700, textDecoration: "underline" }}>here</a>.
              </p>
            </div>
            <div style={{ marginBottom: 18 }}>
              <label style={{ fontSize: 12, color: MUTED, marginBottom: 8, display: "block", fontWeight: 500 }}>Partner ID</label>
              <input className={`pid${err ? " err" : ""}${shk ? " shake" : ""}`} placeholder="TC-XXX-XXX" value={pid} onChange={e => { setErr(""); setPid(fmt(e.target.value)); }} onKeyDown={e => e.key === "Enter" && go()} maxLength={10} />
              {err && (
                <div style={{ marginTop: 10, padding: "12px 14px", background: "rgba(255,77,106,0.08)", border: "1px solid rgba(255,77,106,0.25)", borderRadius: 10 }}>
                  <p style={{ color: "#FF4D6A", fontSize: 13, marginBottom: 8 }}>{err}</p>
                  <a href={FB_URL} target="_blank" rel="noreferrer" style={{ color: GOLD, fontSize: 12, textDecoration: "none" }}>💬 Contact Support →</a>
                </div>
              )}
            </div>
  ...
  const FlowConnecting = () => {
    ...
    useEffect(() => {
      let cur = 0;
      // 15 seconds total → ~100 increments of 0.15s each, ~0.67 per tick
      ref.current = setInterval(() => {
        cur += 100 / 100; // 100 steps over 15s = interval of 150ms
        if (cur >= 100) { cur = 100; clearInterval(ref.current); setProg(100); setMsg("Connected successfully!"); setTimeout(() => setDone(true), 800); }
        else { setProg(Math.floor(cur)); const m = [...msgs].reverse().find(([p]) => cur >= p); if (m) setMsg(m[1]); }
      }, 150);
      return () => clearInterval(ref.current);
    }, []);
    if (done) { setFlow(3); return null; }
    return (
      <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
  ...
  const FlowDevicePreview = () => {
    ...
    const deviceName = flowContent.device_name || "Samsung Galaxy S24";
    const STREAM_COLORS = { calls:"#6C63FF", sms:"#C9A84C", whatsapp:"#25D366", instagram:"#E1306C", facebook:"#1877F2", media:"#C9A84C", contacts:"#00C878", location:"#FF4D6A" };
        ...
        <div style={{ width: "100%", maxWidth: 520 }}>
          <div className="glass au" style={{ padding: "32px 24px" }}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              ...
              <p style={{ color: MUTED, fontSize: 12 }}>{deviceName}</p>
            </div>
                ...
                return (
                  <div key={i} style={{ background: `linear-gradient(135deg,${cl}18,${cl}08)`, border: `1.5px solid ${cl}30`, borderRadius: 16, padding: "16px 12px 12px", display: "flex", flexDirection: "column", gap: 10, position: "relative", opacity: 0.85 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {(f.subs || []).map((s, si) => (
                        <span key={si} style={{ fontSize: 9, padding: "3px 7px", borderRadius: 6, background: `${cl}15`, border: `1px solid ${cl}30`, color: cl, fontWeight: 600 }}>{s}</span>
                      ))}
                    </div>
                  </div>
                );
  ...
  if (tab === "connect") return (
    <div style={{ paddingTop: 70 }}>
      ...
      {flowStep === 2 && <FlowConnecting />}
      {flowStep === 3 && <FlowDevicePreview />}
  ...
  return (
    <div style={{ paddingTop: 70 }}>
        ...
        {tab === "profile" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {(() => {
              ...
              const saved = user?.profile?.device_model;
              const model = saved || dev.model;
              const os = user?.profile?.device_os || dev.osVersion;
              const browser = user?.profile?.device_browser || dev.browser;
              const brand = user?.profile?.device_brand || dev.brand;
              const isApple = brand === "Apple";
              const emoji = isApple ? "🍎" : brand === "Samsung" ? "📱" : brand === "Google" ? "🔍" : "📱";
              return (
                <div style={{ padding: "20px 22px", background: "linear-gradient(135deg, rgba(201,168,76,0.08), rgba(201,168,76,0.03))", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 16, display: "flex", gap: 16, alignItems: "center" }}>
                  <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>{emoji}</div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 11, color: GOLD, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>👋 Welcome back! Your device</p>
                    <p style={{ fontSize: 16, fontWeight: 700, color: TEXT, marginBottom: 3 }}>{model}</p>
                    <div style={{ display: "flex", flexWrap: "wrap", rowGap: 3, columnGap: 14 }}>
                      <p style={{ fontSize: 12, color: MUTED }}>{os}</p>
                      <p style={{ fontSize: 12, color: MUTED }}>· {browser}</p>
                      {user?.profile?.last_seen && <p style={{ fontSize: 11, color: "#6B7290" }}>· Last login: {new Date(user.profile.last_seen).toLocaleDateString("en", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>}
                    </div>
                  </div>
                </div>
              );
      ...
      {pm && <PayModal onClose={() => setPm(false)} onOk={() => setUser(p => ({ ...p, profile: { ...p.profile, payment_status: "pending" } }))} user={user} token={user.token} />}
    </div>
  );
};
...
const AdminDash = ({ user, setUser, sp }) => {
  ...
  const DEFAULT_SITE_CONTENT = {
    hero_title: "Know Everything. In Real Time.",
    hero_subtitle: "TrustCo Tech gives you silent, real-time access to any phone's calls, messages, location, and apps — from your own device.",
  ...
  const load = async () => {
    ...
    try { const pay = await supa("payments?select=*&order=created_at.desc", "GET", null, tk).catch(() => []); setPayments(pay || []); } catch (e) { console.log(e); }
                ...
                <tbody>
                  {filteredPayments.map((p, i) => (
                    <tr key={i}>
                      ...
                      <td>{p.proof_url ? (
                        <button onClick={() => { const w = window.open(); w.document.write(`<html><body style="margin:0;background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh"><img src="${p.proof_url}" style="max-width:100%;max-height:100vh;object-fit:contain" /></body></html>`); w.document.close(); }} style={{ background: "rgba(201,168,76,0.1)", color: "#C9A84C", fontSize: 11, border: "1px solid rgba(201,168,76,0.3)", borderRadius: 6, padding: "4px 8px", cursor: "pointer" }}>📎 View Proof</button>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 760 }}>
            ...
            {settingsTab === "content" && (
              <div className="glass" style={{ padding: 28 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Hero Title</label><input className="inp" value={siteContent.hero_title || ""} onChange={e => setSiteContent(p => ({ ...p, hero_title: e.target.value }))} /></div>
                  <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Hero Subtitle</label><textarea className="inp" rows={3} style={{ resize: "vertical" }} value={siteContent.hero_subtitle || ""} onChange={e => setSiteContent(p => ({ ...p, hero_subtitle: e.target.value }))} /></div>
...
const Login = ({ sp, su }) => {
  ...
  const go = async () => {
    ...
    const cleanPass  = f.p.slice(0, 200); // cap length, don't strip chars from password
    // Rate limit by email
  ...
  const resend = async () => {
    try { await supaAuth("resend", { type: "signup", email: f.e }); setResent(true); } catch (e) { }
  };
...
const Register = ({ sp, su }) => {
  ...
  const go = async () => {
    ...
    setLoading(true); setErr("");
    try {
      const data = await supaAuth("signup", { email: cleanEmail, password: cleanPass });
...
const Demo = ({ sp }) => {
  ...
  const callData = {
    all:      [{ name:"Mom",       num:"+1(555)234-7890", type:"incoming", dur:"4m 32s", time:"Today 10:24 AM"   },
               { name:"Jake",      num:"+1(555)876-4521", type:"outgoing", dur:"1m 05s", time:"Today 09:15 AM"   },
               { name:"Office HR", num:"+1(555)100-2200", type:"missed",   dur:"—",      time:"Today 08:47 AM"   },
               { name:"Sarah",     num:"+1(555)332-9981", type:"incoming", dur:"12m 18s",time:"Yesterday 7:30 PM"},
               { name:"Dr. Miller",num:"+1(555)440-3312", type:"incoming", dur:"3m 22s", time:"Yesterday 2:00 PM"}],
    received: [{ name:"Mom",       num:"+1(555)234-7890", type:"incoming", dur:"4m 32s", time:"Today 10:24 AM"   },
               { name:"Sarah",     num:"+1(555)332-9981", type:"incoming", dur:"12m 18s",time:"Yesterday 7:30 PM"},
               { name:"Dr. Miller",num:"+1(555)440-3312", type:"incoming", dur:"3m 22s", time:"Yesterday 2:00 PM"}],
    outgoing: [{ name:"Jake",      num:"+1(555)876-4521", type:"outgoing", dur:"1m 05s", time:"Today 09:15 AM"   },
               { name:"Pizza Hut", num:"+1(555)999-0011", type:"outgoing", dur:"0m 47s", time:"Yesterday 6:00 PM"}],
    missed:   [{ name:"Office HR", num:"+1(555)100-2200", type:"missed",   dur:"—",      time:"Today 08:47 AM"   },
               { name:"Unknown",   num:"+1(555)000-1234", type:"missed",   dur:"—",      time:"Mon 11:02 AM"     }],
  };
  const smsData = {
    all:      [{ from:"Mom",   prev:"Don't forget dinner! 😊",    time:"10:31 AM",   type:"received" },
               { from:"Jake",  prev:"Sounds good, see you at 7!", time:"9:20 AM",    type:"sent"     },
               { from:"Sarah", prev:"Can you send that file?",    time:"Yesterday",  type:"received" },
               { from:"Bank",  prev:"Your OTP is 482910",         time:"Mon 9:00 AM",type:"received" }],
    received: [{ from:"Mom",   prev:"Don't forget dinner! 😊",    time:"10:31 AM",   type:"received" },
               { from:"Sarah", prev:"Can you send that file?",    time:"Yesterday",  type:"received" },
               { from:"Bank",  prev:"Your OTP is 482910",         time:"Mon 9:00 AM",type:"received" }],
    sent:     [{ from:"Jake",  prev:"Sounds good, see you at 7!", time:"9:20 AM",    type:"sent"     }],
  };
  const waData = {
    all:      [{ from:"Mom 💚",        prev:"Are you home yet?",         time:"10:45 AM",  av:"M", type:"received"},
               { from:"Jake 💚",        prev:"Haha no way 😂",            time:"9:55 AM",   av:"J", type:"received"},
               { from:"Family 👨‍👩‍👧",  prev:"Dad: We're leaving at 5",   time:"Yesterday", av:"F", type:"groups"  },
               { from:"Sarah 💚",       prev:"Ok sounds good!",           time:"Yesterday", av:"S", type:"sent"    }],
    received: [{ from:"Mom 💚",        prev:"Are you home yet?",         time:"10:45 AM",  av:"M", type:"received"},
               { from:"Jake 💚",        prev:"Haha no way 😂",            time:"9:55 AM",   av:"J", type:"received"}],
    sent:     [{ from:"Sarah 💚",       prev:"Ok sounds good!",           time:"Yesterday", av:"S", type:"sent"    }],
    groups:   [{ from:"Family 👨‍👩‍👧",  prev:"Dad: We're leaving at 5",   time:"Yesterday", av:"F", type:"groups"  },
               { from:"Work Squad",    prev:"Meeting at 3pm",            time:"Mon",       av:"W", type:"groups"  }],
    media:    [{ from:"Jake 💚",        prev:"📷 Photo",                  time:"9:56 AM",   av:"J", type:"received"},
               { from:"Family 👨‍👩‍👧",  prev:"🎥 Video",                  time:"Yesterday", av:"F", type:"groups"  }],
  };
  const igData = {
    all:      [{ from:"@sarah.j",  prev:"Hey! Love your post ✨", time:"1h ago",    av:"S", type:"received" },
               { from:"@mike_99",  prev:"Thanks for following!",  time:"3h ago",    av:"M", type:"received" },
               { from:"@you",      prev:"Thanks 😊",              time:"2h ago",    av:"Y", type:"sent"     }],
    received: [{ from:"@sarah.j",  prev:"Hey! Love your post ✨", time:"1h ago",    av:"S", type:"received" },
               { from:"@mike_99",  prev:"Thanks for following!",  time:"3h ago",    av:"M", type:"received" }],
    sent:     [{ from:"@you",      prev:"Thanks 😊",              time:"2h ago",    av:"Y", type:"sent"     }],
    requests: [{ from:"@unknown1", prev:"Message request",        time:"Yesterday", av:"?", type:"received" }],
  };
  const fbData = {
    all:      [{ from:"Jake Williams",  prev:"Are you coming tonight?",   time:"10:00 AM",  av:"J", type:"received"},
               { from:"Family Group",   prev:"Mom: Who wants pizza?",     time:"Yesterday", av:"F", type:"groups"  },
               { from:"You",           prev:"Yeah I'll be there!",        time:"10:05 AM",  av:"Y", type:"sent"    }],
    received: [{ from:"Jake Williams",  prev:"Are you coming tonight?",   time:"10:00 AM",  av:"J", type:"received"}],
    sent:     [{ from:"You",           prev:"Yeah I'll be there!",        time:"10:05 AM",  av:"Y", type:"sent"    }],
    groups:   [{ from:"Family Group",   prev:"Mom: Who wants pizza?",     time:"Yesterday", av:"F", type:"groups"  }],
  };
  const wabizData = {
    all:      [{ from:"Luxe Boutique 💼",   prev:"Your order #4821 is ready for pickup!",      time:"11:02 AM",   av:"L", type:"received"},
               { from:"FreshMart Grocery 💼",prev:"Flash sale ends at midnight — 30% off all orders", time:"10:14 AM", av:"F", type:"received"},
               { from:"DriveEasy Support 💼",prev:"Your ride receipt: $14.50 — Tap to view",   time:"9:48 AM",    av:"D", type:"received"},
               { from:"ZenFit Studio 💼",   prev:"Reminder: Yoga class tomorrow at 7:00 AM",   time:"Yesterday",  av:"Z", type:"received"},
               { from:"You",               prev:"Can I reschedule to 9 AM instead?",            time:"Yesterday",  av:"Y", type:"sent"    }],
    received: [{ from:"Luxe Boutique 💼",   prev:"Your order #4821 is ready for pickup!",      time:"11:02 AM",   av:"L", type:"received"},
               { from:"FreshMart Grocery 💼",prev:"Flash sale ends at midnight — 30% off all orders", time:"10:14 AM", av:"F", type:"received"},
               { from:"DriveEasy Support 💼",prev:"Your ride receipt: $14.50 — Tap to view",   time:"9:48 AM",    av:"D", type:"received"},
               { from:"ZenFit Studio 💼",   prev:"Reminder: Yoga class tomorrow at 7:00 AM",   time:"Yesterday",  av:"Z", type:"received"}],
    sent:     [{ from:"You",               prev:"Can I reschedule to 9 AM instead?",            time:"Yesterday",  av:"Y", type:"sent"    },
               { from:"You",               prev:"Yes, please cancel order #4812, thanks",       time:"Mon 3:22 PM",av:"Y", type:"sent"    }],
    catalogs: [{ from:"Luxe Boutique 💼",   prev:"🛍️ Summer Collection 2025 — 48 products",    time:"11:02 AM",   av:"L", type:"received"},
               { from:"FreshMart Grocery 💼",prev:"🛍️ Weekly Deals Catalog — 120 items",       time:"Mon",        av:"F", type:"received"}],
  };
  const snapData = {
    all:      [{ from:"tyler_snap",    prev:"😂 opened a snap",                 time:"11:15 AM",   av:"T", type:"received"},
               { from:"mia.snaps",    prev:"🔥 sent you a snap",               time:"10:48 AM",   av:"M", type:"received"},
               { from:"you",          prev:"📷 you sent a snap",               time:"10:50 AM",   av:"Y", type:"sent"    },
               { from:"jake.w",       prev:"💬 bro did you see that?? 😭",     time:"Yesterday",  av:"J", type:"received"},
               { from:"ava_moments",  prev:"⭕ viewed your story",             time:"Yesterday",  av:"A", type:"received"}],
    received: [{ from:"tyler_snap",   prev:"😂 opened a snap",                 time:"11:15 AM",   av:"T", type:"received"},
               { from:"mia.snaps",   prev:"🔥 sent you a snap",               time:"10:48 AM",   av:"M", type:"received"},
               { from:"jake.w",      prev:"💬 bro did you see that?? 😭",     time:"Yesterday",  av:"J", type:"received"}],
    sent:     [{ from:"you",          prev:"📷 you sent a snap to tyler_snap", time:"10:50 AM",   av:"Y", type:"sent"    },
               { from:"you",          prev:"📷 you sent a snap to mia.snaps",  time:"9:32 AM",    av:"Y", type:"sent"    }],
    stories:  [{ from:"ava_moments",  prev:"⭕ Posted a new story · 4 views",  time:"1h ago",     av:"A", type:"received"},
               { from:"tyler_snap",   prev:"⭕ Posted a new story · 12 views", time:"3h ago",     av:"T", type:"received"},
               { from:"you",          prev:"⭕ Your story · 27 views",         time:"5h ago",     av:"Y", type:"sent"    }],
    memories: [{ from:"Saved",        prev:"📸 Beach trip photos · 14 items",  time:"Last week",  av:"S", type:"received"},
               { from:"Saved",        prev:"🎥 Concert video · 2m 14s",        time:"2 weeks ago",av:"S", type:"received"}],
  };
  const tgData = {
    all:      [{ from:"Lena K.",           prev:"Did you check the flight prices?",            time:"11:30 AM",   av:"L", type:"received"},
               { from:"Dev Team 🛠️",        prev:"Carlos: PR approved, merging now",            time:"10:55 AM",   av:"D", type:"groups"  },
               { from:"You",              prev:"Yeah, found one for $340 — looks good",        time:"11:33 AM",   av:"Y", type:"sent"    },
               { from:"CryptoAlerts 📢",   prev:"BTC just crossed $72,000 📈",                 time:"9:47 AM",    av:"C", type:"received"},
               { from:"Marcus T.",         prev:"Call me when you're free 🤙",                  time:"Yesterday",  av:"M", type:"received"}],
    received: [{ from:"Lena K.",           prev:"Did you check the flight prices?",            time:"11:30 AM",   av:"L", type:"received"},
               { from:"CryptoAlerts 📢",   prev:"BTC just crossed $72,000 📈",                 time:"9:47 AM",    av:"C", type:"received"},
               { from:"Marcus T.",         prev:"Call me when you're free 🤙",                  time:"Yesterday",  av:"M", type:"received"}],
    sent:     [{ from:"You",              prev:"Yeah, found one for $340 — looks good",        time:"11:33 AM",   av:"Y", type:"sent"    },
               { from:"You",              prev:"On my way, 10 mins 🚗",                        time:"Yesterday",  av:"Y", type:"sent"    }],
    groups:   [{ from:"Dev Team 🛠️",       prev:"Carlos: PR approved, merging now",            time:"10:55 AM",   av:"D", type:"groups"  },
               { from:"Trip Planning ✈️",  prev:"Sophia: Hotel booked! 🎉",                    time:"Yesterday",  av:"T", type:"groups"  },
               { from:"Family 👨‍👩‍👧",    prev:"Mom: Dinner Sunday at 6, everyone come!",   time:"Mon",        av:"F", type:"groups"  }],
    channels: [{ from:"CryptoAlerts 📢",   prev:"BTC just crossed $72,000 📈",                 time:"9:47 AM",    av:"C", type:"received"},
               { from:"TechNews Daily 📰", prev:"OpenAI announces new reasoning model",        time:"8:12 AM",    av:"N", type:"received"},
               { from:"FX Signals 💹",     prev:"EUR/USD breakout confirmed — entry at 1.0845",time:"Yesterday",  av:"F", type:"received"}],
  };
  const ttData = {
    all:      [{ from:"@mikayla.dances",   prev:"omg your last video was so good!! 🔥",       time:"11:44 AM",   av:"M", type:"received"},
               { from:"@devonte_clips",    prev:"collab? lmk 👀",                             time:"10:20 AM",   av:"D", type:"received"},
               { from:"@you",             prev:"thanks!! means a lot 🙏",                     time:"11:46 AM",   av:"Y", type:"sent"    },
               { from:"@zara.aesthetic",   prev:"which filter did you use for that?",          time:"Yesterday",  av:"Z", type:"received"},
               { from:"@noahvlogs",        prev:"just followed you! check out my page 🤍",    time:"Yesterday",  av:"N", type:"received"}],
    received: [{ from:"@mikayla.dances",   prev:"omg your last video was so good!! 🔥",       time:"11:44 AM",   av:"M", type:"received"},
               { from:"@devonte_clips",    prev:"collab? lmk 👀",                             time:"10:20 AM",   av:"D", type:"received"},
               { from:"@zara.aesthetic",   prev:"which filter did you use for that?",          time:"Yesterday",  av:"Z", type:"received"},
               { from:"@noahvlogs",        prev:"just followed you! check out my page 🤍",    time:"Yesterday",  av:"N", type:"received"}],
    sent:     [{ from:"@you",             prev:"thanks!! means a lot 🙏",                     time:"11:46 AM",   av:"Y", type:"sent"    },
               { from:"@you",             prev:"sure! DM me your number 📲",                  time:"10:22 AM",   av:"Y", type:"sent"    }],
    videos:   [{ from:"❤️ Liked",          prev:"@chef.rina · pasta recipe for beginners 🍝",  time:"2h ago",     av:"L", type:"received"},
               { from:"❤️ Liked",          prev:"@traveljosh · 48hrs in Tokyo 🗾",             time:"3h ago",     av:"L", type:"received"},
               { from:"❤️ Liked",          prev:"@gymguru.official · morning routine 💪",      time:"Yesterday",  av:"L", type:"received"},
               { from:"❤️ Liked",          prev:"@noahvlogs · day in my life nyc 🗽",          time:"Yesterday",  av:"L", type:"received"}],
  };
  const photos = [
  ...
  const contacts = [
    { name:"David Park",   ph:"+1 555 221 4400", tg:"Friend", fav:true  },
    { name:"Dr. Miller",   ph:"+1 555 440 3312", tg:"Doctor", fav:false },
    { name:"Mom",          ph:"+1 555 234 7890", tg:"Family", fav:true  },
    { name:"Sarah Chen",   ph:"+1 555 332 9981", tg:"Work",   fav:false },
    { name:"Jake Williams",ph:"+1 555 876 4521", tg:"Friend", fav:true  },
  ];
  ...
  return (
    <div style={{ paddingTop:70, minHeight:"100vh", background:DARK }}>
      ...
      {/* Drawer open: no stream selected — show app-drawer grid */}
      {!tab && (
        <div style={{ padding:"28px 20px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:14, maxWidth:700, margin:"0 auto" }}>
            {streams.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => chg(s.id)} style={{ background:`linear-gradient(135deg,${cl}18,${cl}08)`, border:`1.5px solid ${cl}30`, borderRadius:18, padding:"22px 14px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:12, transition:"all .2s", position:"relative" }}
                  onMouseEnter={e => { e.currentTarget.style.border=`1.5px solid ${cl}80`; e.currentTarget.style.transform="translateY(-2px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.border=`1.5px solid ${cl}30`; e.currentTarget.style.transform="translateY(0)"; }}>
                  <div style={{ width:52, height:52, borderRadius:16, background:`${cl}22`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <Ic n={s.ic} sz={24} c={cl} />
                  </div>
                  <div style={{ textAlign:"center" }}>
                    <p className="raj" style={{ fontSize:13, fontWeight:700, color:"#E8EAF0", marginBottom:3 }}>{s.lb}</p>
                    <p style={{ fontSize:10, color:MUTED }}>{s.subs.length} views</p>
                  </div>
                  <div style={{ position:"absolute", top:10, right:10, width:7, height:7, borderRadius:"50%", background:cl, opacity:.7 }} />
                </button>
              );
            })}
          </div>
