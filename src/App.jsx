        ...
        <div style={{ padding:"28px 20px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:18, maxWidth:900, margin:"0 auto" }}>
            {streams.map(s => {
              ...
              return (
                <div key={s.id} className="glass" style={{ padding: 22, borderRadius: 18, borderColor: cl + "30", background: `linear-gradient(135deg,${cl}10,${cl}04)`, cursor: "pointer" }} onClick={() => chg(s.id)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                      <div style={{ width: 42, height: 42, borderRadius: 12, background: cl + "20", display: "flex", alignItems: "center", justifyContent: "center" }}><Ic n={s.ic} sz={19} c={cl} /></div>
                      <h3 className="raj" style={{ fontSize: 16, fontWeight: 700 }}>{s.lb}</h3>
                    </div>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {s.subs.map(sub => {
                      ...
                      return <span key={id} style={{ padding: "6px 12px", borderRadius: 9, border: `1px solid ${cl}35`, color: cl, fontSize: 12, fontWeight: 600 }}>{lb}</span>;
                    })}
                  </div>
                </div>
              );
