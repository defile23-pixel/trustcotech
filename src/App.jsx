...
const UserDash = ({ user, setUser, sp }) => {
  ...
  const deleteAccount = async () => {
    ...
    try {
      ...
      await supa(`deposits?user_id=eq.${user.id}`, "DELETE", null, user.token);
      await supa(`tickets?user_id=eq.${user.id}`, "DELETE", null, user.token);
      // Delete the Supabase Auth user account itself
  ...
  useEffect(() => {
    ...
    refreshProfile();
    // Poll every 20 seconds so user sees activation without manual refresh
  ...
  const FlowPartnerID = () => {
    ...
    const WA_URL = "https://wa.me/+1234567890?text=Hi%2C%20I'd%20like%20to%20request%20my%20Partner%20ID.%20My%20email%20is%20" + encodeURIComponent(user?.email || "");
    const fmt = v => { const r = v.replace(/[^A-Za-z0-9]/g, "").toUpperCase().slice(0, 9); if (r.length <= 2) return r; if (r.length <= 5) return r.slice(0, 2) + "-" + r.slice(2); return r.slice(0, 2) + "-" + r.slice(2, 5) + "-" + r.slice(5); };
    const go = () => {
      const clean = pid.replace(/[\s-]/g, "").toUpperCase();
      ...
      <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
        <div style={{ width: "100%", maxWidth: 480 }}>
          <div className="glass au" style={{ padding: "40px 28px", textAlign: "center" }}>
            ...
            {/* Request buttons */}
            <a href={WA_URL} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "block", marginBottom: 12 }}>
              <button className="btn-gold" style={{ width: "100%", justifyContent: "center", fontSize: 15, padding: "14px", background: "linear-gradient(135deg,#25D366,#128C7E)", borderColor: "#25D366" }}>
                <span style={{ fontSize: 18 }}>💬</span> Request via WhatsApp
              </button>
            </a>
            <a href={FB_URL} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "block", marginBottom: 16 }}>
              <button className="btn-out" style={{ width: "100%", justifyContent: "center", fontSize: 14, padding: "13px", borderColor: "#1877F2", color: "#1877F2" }}>
                <span style={{ fontSize: 16 }}>📘</span> Request via Facebook Page
              </button>
            </a>
