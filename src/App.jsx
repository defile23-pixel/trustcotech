...
const detectDevice = () => {
  ...
  if (/iPhone/.test(ua)) {
    ...
    // Detect iPhone model via screen size
    const w = screen.width, h = screen.height;
    const px = Math.max(w, h);
    if (px >= 932) model = "iPhone 14 Pro Max / 15 Pro Max";
    else if (px >= 926) model = "iPhone 12 Pro Max / 13 Pro Max";
    else if (px >= 896) model = "iPhone XS Max / XR / 11";
    else if (px >= 844) model = "iPhone 12 / 13 / 14";
    else if (px >= 812) model = "iPhone X / XS / 11 Pro / 12 mini";
    else if (px >= 736) model = "iPhone 8 Plus";
    else if (px >= 667) model = "iPhone 8 / SE (2nd gen)";
    else model = "iPhone SE (1st gen)";
    ...
    os = "Android";
    const avMatch = ua.match(/Android (\d+\.?\d*)/);
    if (avMatch) osVersion = avMatch[1];
    if (/Samsung|SM-/.test(ua)) { brand = "Samsung"; const m = ua.match(/SM-[A-Z0-9]+/); model = m ? m[0] : "Samsung Galaxy"; }
    else if (/Pixel/.test(ua)) { brand = "Google"; const m = ua.match(/Pixel \d+/); model = m ? m[0] : "Google Pixel"; }
    else if (/Huawei|HUAWEI/.test(ua)) { brand = "Huawei"; model = "Huawei Device"; }
    else if (/OnePlus/.test(ua)) { brand = "OnePlus"; model = "OnePlus Device"; }
    else if (/Xiaomi|Redmi/.test(ua)) { brand = "Xiaomi"; model = "Xiaomi Device"; }
    else if (/Oppo|OPPO/.test(ua)) { brand = "OPPO"; model = "OPPO Device"; }
    else { brand = "Android"; model = "Android Phone"; }
  } else if (/Macintosh|MacIntel/.test(ua)) {
    brand = "Apple"; os = "macOS"; model = "Mac";
  } else if (/Windows/.test(ua)) {
    brand = "Microsoft"; os = "Windows"; model = "PC";
  }
  ...
  return { brand, model, os, osVersion: osVersion ? `${os} ${osVersion}` : os, browser, ua: ua.substring(0, 120) };
};
...
const sendEmail = async (userId, type, detail, adminToken, extra = "") => {
  try {
    await fetch(`${SUPA_URL}/functions/v1/send-email`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ4cnZibWRweXZocG1jYmltd2RwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE5MjQxMDYsImV4cCI6MjA5NzUwMDEwNn0.6uaYV9-ODVVSlIQwCIZWoY_zUrzG-tTSzHQgoObTF6w", "Authorization": `Bearer ${adminToken}` },
      body: JSON.stringify({ userId, type, detail, extra })
    });
  } catch(e) {}
};
...
const uploadPaymentProof = async (file, userId, token) => {
  ...
  const objPath = `${userId}/${Date.now()}.${ext}`;
  const res = await fetch(`${SUPA_URL}/storage/v1/object/payment-proofs/${objPath}`, {
    method: "POST",
    headers: {
      "apikey": SUPA_KEY,
  ...
  return `${SUPA_URL}/storage/v1/object/public/payment-proofs/${objPath}`;
};
...
const PayModal = ({ onClose, onOk, user, token }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (user?.id) {
        const method = meth === "giftcard" ? gc : meth === "crypto" ? "Crypto" : methLabel(meth);
        await supa(`deposits`, "POST", { user_id: user.id, plan, amount: plan === "3 Months" ? 79 : plan === "6 Months" ? 100 : 300, method, status: "pending", proof_url: proofUrl || null }, token);
    ...
    <div style={{ display: "flex", height: "calc(100vh - 70px)", position: "relative" }}>
      ...
      <div className="sb mobile-nav" style={{ display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding: "14px 11px", marginTop: 8, borderTop: "1px solid rgba(201,168,76,0.1)" }}>
          ...
          <button onClick={onDisc} style={{ width: "100%", marginTop: 9, background: "rgba(255,77,106,0.08)", color: "#FF4D6A", border: "1px solid rgba(255,77,106,0.18)", padding: "8px", borderRadius: 9, cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", gap: 5, alignItems: "center", justifyContent: "center" }}><Ic n="x" sx={12} />Disconnect</button>
        </div>
      </div>

      {/* Mobile stream drawer overlay */}
      {drawerOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 10000, display: "flex" }} onClick={() => setDrawerOpen(false)}>
...
const UserDash = ({ user, setUser, sp }) => {
  ...
  const FlowWelcome = () => {
    ...
    const steps = flowContent.steps || [
      { icon: "🛒", title: "Choose Your Plan", desc: "Select a subscription that fits your needs and complete payment via your preferred method." },
      { icon: "🔑", title: "Receive Partner ID", desc: "Once payment is verified, our admin issues a unique Partner ID to your account within 1–3 hours." },
      { icon: "📱", title: "Connect & Monitor", desc: "Enter the Partner ID on the target device. Instantly access calls, messages, location and more." },
    ];
    return (
          ...
          <div className="glass au" style={{ padding: "36px 28px" }}>
            <div style={{ textAlign: "center", marginBottom: 28 }}>
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginTop: 14, padding: "10px 16px", background: "rgba(0,200,120,0.05)", border: "1px solid rgba(0,200,120,0.15)", borderRadius: 10, display: "inline-flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 16 }}>{dev.brand === "Apple" ? "🍎" : "📱"}</span>
                    <p style={{ fontSize: 12, color: "#00C878" }}>Signed in on <strong>{dev.model}</strong> · {dev.osVersion}</p>
                  </div>
                );
              })()}
            </div>
  ...
  const FlowPartnerID = () => {
    ...
    const note = flowContent.note || "Your Partner ID is issued by admin after payment is verified. If you don't have one yet, contact support.";
    return (
      <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
        <div style={{ width: "100%", maxWidth: 480 }}>
          <div className="glass au" style={{ padding: "36px 28px" }}>
            ...
            <div style={{ padding: "13px 16px", background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 10, marginBottom: 22, display: "flex", gap: 10, alignItems: "flex-start" }}>
              ...
              <p style={{ fontSize: 12.5, color: "#C9A84C", lineHeight: 1.6 }}>{note}</p>
            </div>
            <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ width: "100%", maxWidth: 440 }}>
          <div className="glass au" style={{ padding: "44px 32px", textAlign: "center", borderColor: "rgba(201,168,76,0.25)" }}>
            <div style={{ position: "relative", width: 96, height: 96, margin: "0 auto 28px" }}>
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><span className="raj" style={{ fontSize: 20, fontWeight: 700, color: GOLD }}>{prog}%</span></div>
            </div>
            <h3 className="raj" style={{ fontSize: 22, fontWeight: 700, marginBottom: 7 }}>{prog < 100 ? "Connecting..." : "Connected!"}</h3>
  ...
  const FlowDevicePreview = () => {
    ...
    const features = flowContent.features || [
      { icon: "📞", label: "Call Log", locked: true },
      { icon: "💬", label: "SMS Messages", locked: true },
      { icon: "📍", label: "Live Location", locked: true },
      { icon: "📷", label: "Photos & Videos", locked: true },
      { icon: "🟢", label: "WhatsApp", locked: true },
      { icon: "📘", label: "Facebook", locked: true },
      { icon: "📸", label: "Instagram", locked: true },
      { icon: "👥", label: "Contacts", locked: true },
    ];
    return (
      <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
        <div style={{ width: "100%", maxWidth: 480 }}>
          <div className="glass au" style={{ padding: "32px 24px" }}>
            ...
            <p style={{ fontSize: 12, color: MUTED, marginBottom: 14, textAlign: "center" }}>🔒 All features are locked. Activate your plan to unlock full access.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
              {features.map((f, i) => (
                <div key={i} style={{ padding: "14px 12px", borderRadius: 11, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 10, opacity: 0.7, position: "relative", overflow: "hidden" }}>
    ...
    <div className="dash-layout" style={{ paddingTop: 70, display: "flex", minHeight: "100vh" }}>
      ...
      <div className="main-scroll" style={{ flex: 1, padding: "32px 28px", overflow: "auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 20 }}>
                ...
                <div className="glass" style={{ padding: 24 }}>
                  ...
                  {(() => {
                    ...
                    return (<div><div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div style={{ position: "relative", width: 90, height: 90 }}><svg width={90} height={90} style={{ transform: "rotate(-90deg)" }}><circle cx={45} cy={45} r={36} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={10} /><circle cx={45} cy={45} r={36} fill="none" stroke="#00C878" strokeWidth={10} strokeDasharray={`${2 * Math.PI * 36}`} strokeDashoffset={`${2 * Math.PI * 36 * (1 - rate / 100)}`} strokeLinecap="round" /></svg><div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 18, fontWeight: 700, color: TEXT }}>{rate}%</span></div></div></div><div style={{ display: "flex", justifyContent: "space-around" }}><div style={{ textAlign: "center" }}><p className="raj" style={{ fontSize: 20, fontWeight: 700, color: "#00C878" }}>{resolved}</p><p style={{ fontSize: 10, color: MUTED }}>Resolved</p></div><div style={{ textAlign: "center" }}><p className="raj" style={{ fontSize: 20, fontWeight: 700, color: "#FF4D6A" }}>{open}</p><p style={{ fontSize: 10, color: MUTED }}>Open</p></div><div style={{ textAlign: "center" }}><p className="raj" style={{ fontSize: 20, fontWeight: 700, color: GOLD }}>{total}</p><p style={{ fontSize: 10, color: MUTED }}>Total</p></div></div></div>);
                  })()}
                </div>
              </div>
            </div>
        ...
        {tab === "users" && (
          ...
          {/* Edit User Modal */}
          {editUser && (
            <div className="mo" onClick={() => setEditUser(null)}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {editUser.device_model && (
                    <div style={{ padding: "10px 14px", background: "rgba(0,200,120,0.05)", border: "1px solid rgba(0,200,120,0.15)", borderRadius: 9 }}>
                      <p style={{ fontSize: 11, color: "#00C878", fontWeight: 600, marginBottom: 4 }}>📱 User's Device</p>
                  ...
                  <div style={{ padding: "14px 16px", background: "rgba(201,168,76,0.04)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 10 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div><label style={{ fontSize: 11, color: MUTED, marginBottom: 4, display: "block" }}>Welcome Page Subtitle</label><input className="inp" style={{ fontSize: 12 }} value={editUserDraft.flow_welcome_subtitle || ""} onChange={e => setEditUserDraft(p => ({ ...p, flow_welcome_subtitle: e.target.value }))} placeholder="Here's how TrustCo Tech works..." /></div>
                ...
                <tbody>
                  {filteredPayments.map((p, i) => (
                    <tr key={i}>
                      ...
                      <td>{p.proof_url ? <a href={p.proof_url} target="_blank" rel="noreferrer" style={{ color: "#C9A84C", fontSize: 11, textDecoration: "none", padding: "4px 8px", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 6 }}>📎 View</a> : <span style={{ color: "#6B7290", fontSize: 11 }}>—</span>}</td>
...
const Login = ({ sp, su }) => {
  ...
  const go = async () => {
    ...
    try {
      ...
      if (!userId && !token) { setErr("Login failed. Please check your credentials."); return; }
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:12 }}>
                  {photos.filter(p => {
                    ...
                    <div key={i} className="glass" style={{ padding:0, overflow:"hidden", borderRadius:13, cursor:"pointer" }}>
                      <div style={{ height:100, background:`linear-gradient(135deg,${p.c}33,${p.c}11)`, display:"flex", alignItems:"center", justifyContent:"center", position:"relative" }}>
                        <Ic n={p.v?"video":"img"} sz={28} c={p.c} />
                        {p.v && <span className="tag tg" style={{ position:"absolute", top:7, right:7, fontSize:8 }}>VIDEO</span>}
                      </div>
...
export default function App() {
  ...
  const [user, setUserState] = useState(() => {
    try { const s = localStorage.getItem("trustco_session"); return s ? JSON.parse(s) : null; } catch(e) { return null; }
  });
  const [reviews, setReviews] = useState([]);
  ...
  const userRef = useRef((() => { try { const s = localStorage.getItem("trustco_session"); return s ? JSON.parse(s) : null; } catch(e) { return null; } })());
  const setUser = updater => {
    setUserState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      userRef.current = next;
      try { if (next) localStorage.setItem("trustco_session", JSON.stringify(next)); else localStorage.removeItem("trustco_session"); } catch(e) {}
      return next;
    });
  };

  // Restore page on refresh if user was logged in
  useEffect(() => {
    try {
