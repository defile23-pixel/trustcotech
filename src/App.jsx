...
const uploadPaymentProof = async (file, userId, token) => {
  ...
  const res = await fetch(`${SUPA_URL}/storage/v1/object/payment-proofs/${path}`, {
    method: "POST",
    headers: { "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ4cnZibWRweXZocG1jYmltd2RwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE5MjQxMDYsImV4cCI6MjA5NzUwMDEwNn0.6uaYV9-ODVVSlIQwCIZWoY_zUrzG-tTSzHQgoObTF6w", "Authorization": `Bearer ${token}`, "Content-Type": file.type },
    body: file
  });
  if (!res.ok) throw new Error("Upload failed");
  return `${SUPA_URL}/storage/v1/object/public/payment-proofs/${path}`;
};
...
const NotifBell = ({ userId, token }) => {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: 42, width: 300, background: "#161929", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 14, zIndex: 9999, boxShadow: "0 8px 32px rgba(0,0,0,0.4)", overflow: "hidden" }}>
...
@media(max-width:768px){
  ...
  .sb.mobile-nav{position:fixed!important;left:0!important;bottom:0!important;top:auto!important;width:100%!important;flex-direction:row!important;overflow-x:auto!important;z-index:99999!important;border-top:1px solid rgba(201,168,76,0.2)!important;border-right:none!important;padding:6px 6px!important;gap:2px!important;max-height:72px!important;min-height:60px!important;min-width:unset!important;background:#0D1020!important;-webkit-overflow-scrolling:touch!important;scrollbar-width:none!important;}
  .sb.mobile-nav::-webkit-scrollbar{display:none!important;}
  .sb.mobile-nav .sl{flex-direction:column!important;gap:2px!important;font-size:9px!important;padding:5px 7px!important;min-width:48px!important;flex-shrink:0!important;border-radius:10px!important;border-left:none!important;white-space:nowrap!important;}
  .sb.mobile-nav > div:last-child{display:none!important;}
  .main-scroll{padding-bottom:88px!important;}
}
...
const UserDash = ({ user, setUser, sp }) => {
  ...
  if (tab === "connect") return (
    <div style={{ paddingTop: 70 }}>
      <TopBar />
      {/* Onboarding progress bar (only show if not fully connected) */}
      {stage !== "ready_to_connect" && (
        <div style={{ maxWidth: 800, margin: "28px auto 0", padding: "0 24px" }}>
          <div className="glass" style={{ padding: "20px 24px" }}>
            <p className="raj" style={{ fontSize: 13, color: GOLD, fontWeight: 700, letterSpacing: ".08em", marginBottom: 16 }}>GETTING STARTED</p>
            <div style={{ display: "flex", gap: 0, position: "relative" }}>
              <div style={{ position: "absolute", top: 14, left: 14, right: 14, height: 2, background: "rgba(255,255,255,0.06)", zIndex: 0 }} />
              <div style={{ position: "absolute", top: 14, left: 14, height: 2, zIndex: 1, background: `linear-gradient(90deg,${GOLD},#9A7B2F)`, width: `${(onboardingSteps.filter(s => s.done).length / (onboardingSteps.length - 1)) * 100}%`, transition: "width .6s ease" }} />
              {onboardingSteps.map((s, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, zIndex: 2, position: "relative" }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: s.done ? `linear-gradient(135deg,${GOLD},#9A7B2F)` : "rgba(255,255,255,0.06)", border: s.done ? "none" : "2px solid rgba(255,255,255,0.1)", transition: "all .3s" }}>
                    {s.done ? <Ic n="check" sz={13} c={DARK} /> : <span style={{ fontSize: 10, color: MUTED, fontWeight: 700 }}>{i + 1}</span>}
                  </div>
                  <p style={{ fontSize: 9, color: s.done ? TEXT : MUTED, textAlign: "center", lineHeight: 1.4, maxWidth: 60 }}>{s.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {view === "connect" ? (
        stage === "ready_to_connect" ? <Connect user={user} onDone={() => setView("activity")} /> : <Welcome user={user} stage={stage} onActivate={() => setPm(true)} />
      ) : <Activity user={user} onDisc={() => setView("connect")} setPM={setPm} />}
                    ...
                    <div key={i} style={{ padding: "16px 18px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10 }}>
                      ...
                      {t.reply && (
                        <div style={{ padding: "10px 13px", background: "rgba(0,200,120,0.06)", border: "1px solid rgba(0,200,120,0.18)", borderRadius: 8, marginTop: 8 }}>
                          <p style={{ fontSize: 10, color: "#00C878", marginBottom: 4 }}>Admin reply:</p>
                          <p style={{ fontSize: 12, color: "#A8B0C2" }}>{t.reply}</p>
                        </div>
                      )}
                    </div>
                  ))}
...
const AdminDash = ({ user, setUser, sp }) => {
  ...
  const [planList, setPlanList] = useState(() => {
    try { const s = localStorage.getItem("admin_plans"); return s ? JSON.parse(s) : DEFAULT_PLANS; } catch(e) { return DEFAULT_PLANS; }
  });
  ...
  const savePlans = (updated) => {
    setPlanList(updated);
    try { localStorage.setItem("admin_plans", JSON.stringify(updated)); } catch(e) {}
  ...
  const load = async () => {
    setLoading(true);
    const timeout = setTimeout(() => setLoading(false), 8000); // safety: never stay loading > 8s
    const tk = user.token;
  ...
  const banUser = async uid => {
    try { await supa(`profiles?id=eq.${uid}`, "PATCH", { activated: false, banned: true }, user.token); showToast("User banned"); setBanConfirm(null); load(); } catch (e) { showToast("Error: " + e.message); }
  };

  const unbanUser = async uid => {
    try { await supa(`profiles?id=eq.${uid}`, "PATCH", { banned: false }, user.token); showToast("User unbanned"); load(); } catch (e) { showToast("Error: " + e.message); }
  };
      ...
      <div className="main-scroll" style={{ flex: 1, padding: "32px 28px", overflow: "auto" }}>
              ...
              <table className="tbl">
                <thead>
                  <tr>
                    ...
                    <th>Name</th><th>Email</th><th>Partner ID</th><th>Plan</th><th>Status</th><th>Joined</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {tab === "settings" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 700 }}>
            <h1 className="raj" style={{ fontSize: 24, fontWeight: 700 }}>Admin Settings</h1>
            <div className="glass" style={{ padding: 28 }}>
            ...
            <div className="glass" style={{ padding: 28 }}>
              <h3 className="raj" style={{ fontSize: 17, fontWeight: 700, marginBottom: 18 }}>Site Configuration</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Support Email</label><input className="inp" value={settings.supportEmail} onChange={e => setSettings(p => ({ ...p, supportEmail: e.target.value }))} placeholder="support@example.com" /></div>
                <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Facebook Support URL</label><input className="inp" value={settings.fbUrl} onChange={e => setSettings(p => ({ ...p, fbUrl: e.target.value }))} placeholder="https://facebook.com/..." /></div>
                <div><label style={{ fontSize: 12, color: MUTED, marginBottom: 5, display: "block" }}>Announcement Banner <span style={{ fontWeight: 400, color: MUTED }}>(shown site-wide)</span></label><input className="inp" value={settings.announcement} onChange={e => setSettings(p => ({ ...p, announcement: e.target.value }))} placeholder="e.g. Maintenance Sunday 2AM UTC" /></div>
                <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: "rgba(255,77,106,0.05)", border: "1px solid rgba(255,77,106,0.15)", borderRadius: 10 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: TEXT, marginBottom: 2 }}>Maintenance Mode</p>
                    <p style={{ fontSize: 11, color: MUTED }}>Only admins can access the site when enabled.</p>
                  </div>
                  <div onClick={() => setSettings(p => ({ ...p, maintenanceMode: !p.maintenanceMode }))} style={{ width: 44, height: 24, borderRadius: 12, background: settings.maintenanceMode ? "#FF4D6A" : "rgba(255,255,255,0.1)", cursor: "pointer", position: "relative", transition: "background .2s", flexShrink: 0 }}>
                    <div style={{ position: "absolute", top: 3, left: settings.maintenanceMode ? 22 : 3, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left .2s" }} />
                  </div>
                </div>
                <button className="btn-gold" style={{ alignSelf: "flex-start" }} onClick={() => { setSettingsSaved(true); setTimeout(() => setSettingsSaved(false), 2500); showToast("Settings saved"); }}>{settingsSaved ? "Saved!" : "Save Settings"}</button>
              </div>
            </div>
          </div>
        )}
...
const Demo = ({ sp }) => {
  ...
  const DEFAULT_STREAMS = [
    { id: "calls",     lb: "Call Log",       ic: "phone",     subs: [
      { id: "all",      lb: "All Calls",       icon: "📋" },
      { id: "received", lb: "Received",        icon: "📞" },
      { id: "outgoing", lb: "Outgoing",        icon: "📤" },
      { id: "missed",   lb: "Missed",          icon: "❌" },
    ]},
    { id: "sms",      lb: "SMS",             ic: "msg",       subs: [
      { id: "all",      lb: "All Messages",    icon: "💬" },
      { id: "received", lb: "Received",        icon: "📥" },
      { id: "sent",     lb: "Sent",            icon: "📤" },
    ]},
    { id: "whatsapp", lb: "WhatsApp",        ic: "whatsapp",  subs: [
      { id: "all",      lb: "All Chats",       icon: "💚" },
      { id: "received", lb: "Received",        icon: "📥" },
      { id: "sent",     lb: "Sent",            icon: "📤" },
      { id: "groups",   lb: "Groups",          icon: "👥" },
      { id: "media",    lb: "Media",           icon: "🖼️" },
    ]},
    { id: "instagram",lb: "Instagram",       ic: "instagram", subs: [
      { id: "all",      lb: "All DMs",         icon: "💌" },
      { id: "received", lb: "Received",        icon: "📥" },
      { id: "sent",     lb: "Sent",            icon: "📤" },
      { id: "requests", lb: "Requests",        icon: "📬" },
    ]},
    { id: "facebook", lb: "Facebook",        ic: "facebook",  subs: [
      { id: "all",      lb: "All Messages",    icon: "💬" },
      { id: "received", lb: "Received",        icon: "📥" },
      { id: "sent",     lb: "Sent",            icon: "📤" },
      { id: "groups",   lb: "Groups",          icon: "👥" },
    ]},
    { id: "media",    lb: "Photos & Videos", ic: "img",       subs: [
      { id: "all photos",    lb: "All Photos",     icon: "🖼️" },
      { id: "camera",        lb: "Camera Roll",    icon: "📷" },
      { id: "screenshots",   lb: "Screenshots",    icon: "🖥️" },
      { id: "all videos",    lb: "All Videos",     icon: "🎬" },
      { id: "recorded",      lb: "Recorded",       icon: "🎥" },
      { id: "downloaded",    lb: "Downloaded",     icon: "⬇️" },
    ]},
    { id: "contacts", lb: "Contacts",        ic: "users",     subs: [
      { id: "all",      lb: "All Contacts",    icon: "👤" },
      { id: "favorites",lb: "Favorites",       icon: "⭐" },
      { id: "recent",   lb: "Recent",          icon: "🕐" },
    ]},
    { id: "location", lb: "Live Location",   ic: "map",       subs: [
      { id: "live",     lb: "Live GPS",        icon: "📍" },
      { id: "history",  lb: "History",         icon: "🗺️" },
      { id: "zones",    lb: "Safe Zones",      icon: "🏠" },
    ]},
  ];
  ...
  const photos = [
    { n:"IMG_2041.jpg", s:"3.2 MB", d:"Today",     c:GOLD,    cat:"camera"      },
    { n:"IMG_2040.jpg", s:"2.8 MB", d:"Today",     c:SILVER,  cat:"camera"      },
    { n:"Screenshot.png",s:"1.1 MB",d:"Yesterday", c:"#6C63FF",cat:"screenshots"},
    { n:"VID_0291.mp4", s:"48 MB",  d:"Yesterday", c:"#FF4D6A",cat:"recorded", v:true },
    { n:"IMG_2039.jpg", s:"4.1 MB", d:"Yesterday", c:"#00C878",cat:"camera"    },
    { n:"VID_0290.mp4", s:"112 MB", d:"Mon",       c:GOLD,    cat:"downloaded", v:true},
  ];
  const contacts = [
